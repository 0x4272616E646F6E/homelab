locals {
  ...
  machine_patch = yamlencode({
    machine = {
      network    = local.machine_network
      kubelet    = local.machine_kubelet
      features   = local.machine_features
      nodeLabels = local.machine_node_labels
      install    = local.machine_install
      disks      = local.machine_disks
      kernel     = local.machine_kernel
      sysctls    = local.machine_sysctls
      sysfs      = local.machine_sysfs
      time       = local.machine_time
      registries = local.machine_registries
    }
  })
  ...
  cluster_patch = yamlencode({
    cluster = {
      apiServer                      = local.cluster_api_server
      controllerManager              = local.cluster_controller_manager
      proxy                          = { disabled = true }
      scheduler                      = local.cluster_scheduler
      discovery                      = local.cluster_discovery
      etcd                           = local.cluster_etcd
